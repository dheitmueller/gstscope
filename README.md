...
- Pan, zoom, fit, re-layout, node dragging, search, and a draggable minimap showing the current viewport.
- In-browser raw Graphviz comparison using the same DOT source.
- Fifteen GPL-compatible, attributed sample pipelines, including three
  unmodified real-world GStreamer 1.26 captures plus bin-rich, RTP, compositor,
  tee, playback, and stress topologies.
...
The sample menu contains three unmodified real GStreamer 1.26 runtime captures
from the GPL-3.0-or-later GstPipelineStudio project, eleven compact DOT fixtures
generated by this project from pipelines documented by the GStreamer Project,
and GstScope's synthetic stress graph. Each sample's author, source reference,
license, and description appear in the inspector when it is selected. The
GstScope-generated fixtures are licensed GPL-2.0-or-later.

Third-party DOT samples are stored byte-for-byte as received; their license and
attribution metadata live in the catalog rather than being added to or rewritten
inside the samples. The build validation pins their upstream SHA-256 hashes.
